...
async function verifyPIIWithGemini(postText, entities) {
  ...
  try {
    ...
    if (!text) {
      logCheck({ status: 'unavailable', reason: 'empty response from Gemini', model, rawResponse, postText, entities })
      return 'unavailable'
    }
    ...
    if (typeof result.is_pii !== 'boolean') {
      logCheck({ status: 'unavailable', reason: 'is_pii not a boolean', model, geminiResponse: text, postText, entities })
      return 'unavailable'
    }

    const status = result.is_pii ? 'confirmed' : 'false_positive'
    logCheck({ status, model, reason: result.reason, postText, entities, geminiResponse: text })
    return status
  } catch (error) {
