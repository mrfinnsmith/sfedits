...
// Per-request timeout. Pro can think past this; on timeout we retry on Flash.
...
/**
 * Build the generateContent request body for a given model.
 * Flash models support disabling thinking (thinkingBudget: 0), which keeps
 * the fallback fast. Pro rejects budget 0 ("only works in thinking mode"),
 * so it must be left on its default thinking behavior.
 */
 ...
 */
async function verifyPIIWithGemini(postText, entities, articleName) {
  const apiKey = process.env.GEMINI_API_KEY
  if (!apiKey) {
    logCheck({ status: 'unavailable', reason: 'GEMINI_API_KEY not set', postText, entities, articleName })
    return 'unavailable'
  }
  ...
  try {
    ...
    try {
      ...
      rawResponse = await makeRequest(reqPath, buildRequestBody(model, prompt))
    } catch (error) {
      // Pro can think past the 10s limit. If it times out, retry this same
      // check once on Flash (fast, thinking off) before giving up and blocking.
      if (error.isTimeout && model !== MODEL_FLASH) {
        console.log(`⚠ ${model} timed out, falling back to ${MODEL_FLASH}`)
        model = MODEL_FLASH
        const fallbackPath = `/v1beta/models/${MODEL_FLASH}:generateContent?key=${apiKey}`
        rawResponse = await makeRequest(fallbackPath, buildRequestBody(model, prompt))
      } else {
        throw error
      }
    }
  } catch (error) {
    logCheck({ status: 'unavailable', reason: error.message, model, postText, entities, articleName })
    return 'unavailable'
  }
  ...
  try {
    ...
    if (!text) {
      logCheck({ status: 'unavailable', reason: 'empty response from Gemini', model, rawResponse, postText, entities, articleName, ...tokenInfo })
      return 'unavailable'
    }
    ...
    if (typeof result.is_pii !== 'boolean') {
      logCheck({ status: 'unavailable', reason: 'is_pii not a boolean', model, geminiResponse: text, postText, entities, articleName, ...tokenInfo })
      return 'unavailable'
    }

    const status = result.is_pii ? 'confirmed' : 'false_positive'
    logCheck({ status, model, reason: result.reason, postText, entities, articleName, geminiResponse: text, ...tokenInfo })
    return status
  } catch (error) {
    logCheck({ status: 'unavailable', reason: 'failed to parse response: ' + error.message, model, rawResponse, postText, entities, articleName })
    return 'unavailable'
  }
}
